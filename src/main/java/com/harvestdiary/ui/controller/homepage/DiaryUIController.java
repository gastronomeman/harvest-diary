...
public class DiaryUIController {
    ...
    void saveDiary(MouseEvent event) {
        if(diaryText.getText().isEmpty() || title.getText().isEmpty()){
            showAlert("标题和内容不能为空");
            return;
        }
        diary.setTime(datePicker.getValue().toString());//设置时间
        diary.setFontSize(String.valueOf(fontSize));
...
//        RecordMarker.CheckFolder();
    ...
    private void setIcon() {
        ...
        save.setGraphic(SettingFontIcon.setSizeAndColor(AntDesignIconsOutlined.SAVE, 20, Color.web("#617172")));
        putToCloud.setGraphic(SettingFontIcon.setSizeAndColor(AntDesignIconsOutlined.CLOUD_SYNC, 20, Color.web("#617172")));
    }
    //格式化DataPicker
    ...
    private void readDiary(HashMap<String, String> hm){
        ...
        if (!(diaryPath == null)){
            ...
            datePicker.setValue(LocalDate.parse(d.getTime()));
            ...
            diaryText.setFont(Font.font(diary.getFontSize()));
